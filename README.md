...
## Phase 7 — Duplicate ID bug, login-stuck bug, and download/share speed

**Found and fixed a real, concrete bug: duplicate `id="btn-add-product"`.**
Two different buttons shared that id — the "+ Add product" nav button on
Builder Market, and the actual submit button inside the Add Product form.
`getElementById`/`querySelector('#id')` always returns the *first* match
in the document, so the submit handler's loading-state code was toggling
the spinner on the wrong (hidden, different-screen) button the whole
time — that's exactly why no loading indicator ever showed while a
product uploaded. Renamed the nav button to `btn-add-product-nav`; the
form's submit button keeps the original id. Checked the entire file for
any other id collisions — there are none.

**Add Product now shows real progress**: the button label itself changes
to "Uploading…" during the operation (not just a spinner), and the
post-save cleanup (resetting the form, navigating to Builder Market) is
now in its own try/catch separate from the actual save — so a cosmetic
UI hiccup after a successful save can never masquerade as "upload
failed" when the product was actually saved fine.

**Hardened error messages app-wide**: raw JavaScript runtime errors (like
"Cannot read properties of null…") can no longer reach the user in a
toast. `uploadErrorMessage()` now only surfaces messages we deliberately
authored ourselves; genuine `TypeError`s get logged to the console for
debugging and show a friendly fallback instead. If that exact error
recurs, the real cause will be in your browser console (DevTools ->
Console tab) even though the user just sees a normal-sounding message —
worth checking there if it happens again so I can find the precise line.

**Login-stuck-on-auth-screen bug, fixed at the root.** After a successful
sign-in, the code re-fetched the user's Firestore profile once; if that
single read failed (a real, known class of Firebase timing issue —
Firestore can briefly evaluate a read as unauthenticated in the instant
right after sign-in, before Auth state has fully propagated to it), the
profile stayed `null` forever with nothing to retry it, and `render()`
would fall back to showing the login screen — even though sign-in had
genuinely succeeded (hence the "Welcome back" toast with no navigation).
Two changes:
- Profile loading now retries up to 3 times with a short backoff before
  giving up, smoothing over that timing window entirely.
- If the profile still isn't loaded, the screen no longer silently
  reverts to Welcome/Login (which looked identical to "nothing happened")
  — it stays put and shows a clear error toast instead.
- As a second layer of defense: `render()`'s screen-setup dispatch is now
  wrapped in try/catch, so if any individual screen's setup function ever
  throws, it can't silently freeze navigation for the whole app the way
  it could before.

**Download/Share were correctness-fine but felt broken — fixed the
actual slowness.** `shareMedia()` used to try attaching the *entire media
file* to the share sheet first, which meant downloading a whole video or
audio file into memory before the share sheet even opened — that's the
real "taking long to respond." Reordered: link-sharing (near-instant, and
every target — Facebook, Instagram, WhatsApp, X — handles a shared link
fine) is now tried first; file-attachment is only attempted for images
(small, fast) as a secondary enhancement, never for video/audio. Both
`downloadMedia()` and `shareMedia()` now also visibly disable the tapped
button and swap its label to "Preparing…"/"Sharing…" immediately, so
there's instant visual feedback instead of the button just sitting there
looking unresponsive while work happens in the background.

## Phase 10 — Dedicated Reels, autoplay rescoped, profile identity hardened

**Autoplay rescoped exactly as asked.** The mixed "For You" feed (photos,
audio, video together) no longer autoplays video by default — it now
shows a tap-to-play button, off unless the person explicitly turns
"Autoplay videos in the For You feed" on from Edit Profile. Reels (below)
is the one place that always autoplays, same as Facebook: entering a
video-only space is treated as opting into video, browsing a mixed feed
isn't.

**Built a real, dedicated Reels section** (`#/reels`, reached via the new
🎬 Reels tab in Explore) — genuinely full-screen, no topbar or bottom
nav, video-only:
- Vertical scroll-snap between videos (CSS `scroll-snap-type`, not a
  gesture library) — one video fills the screen at a time.
- A single shared `IntersectionObserver` always autoplays whichever video
  is in view and pauses every other one — same efficient pattern as the
  feed's autoplay, just without the opt-in gate.
- Quietly loads the next page of Reels as you approach the last loaded
  video, so scrolling never hits a hard stop.
- Right-side vertical action stack (heart/like with count, comment with
  count, share, download, and a "⋯" report/block menu) matching the
  Facebook Reels layout you shared earlier — reusing the exact same
  `toggleReaction()`, `shareMedia()`, `downloadMedia()`, and
  report/block logic already built for the feed, not reimplemented.
- **Reels-style comments**: a dark, rounded bottom sheet that slides up
  over the video (not the lightbox's page-style panel) — comment list,
  input pinned at bottom, swipe-down-to-close feel. Visible to and
  postable by both Community and Builders, with the same per-comment
  Report and owner/author Delete actions as everywhere else. Comment
  counts sync back to the reel card live as you post/delete.
- Mute state persists across videos within a viewing session (mute once,
  stay muted as you scroll) rather than resetting per-video — matches
  how TikTok/Instagram actually behave.
- Blocked users are filtered out here too, same as Discover and the feed.

**Profile identity hardened.** Investigated the "wrong profile showing"
report and found a real gap: several caches (blocked-users list, follow
list, feed indexes) were module-level variables that never reset when a
*different* account signed in on the same tab — exactly the class of bug
that causes stale/wrong data on a shared or reused device. Fixed with a
`resetPerSessionCaches()` call on every sign-out and on every genuinely
new sign-in (including Firebase Auth's own cross-tab session sync, not
just an explicit logout button). On top of that, the Profile screen now
renders instantly from the cached copy (no blank flash) and then quietly
re-confirms against a fresh server read — closing any staleness window
regardless of its cause, not just the one found.

**Advanced profile identity on login.** Both dashboards now show a real
profile-photo avatar (with initials fallback) next to your name right
when you land after logging in, not just plain text — Builder's dashboard
also shows a live "online" ring since it's always you, obviously online.

## Phase 9 — Scroll-triggered video autoplay + finishing the anti-abuse system

**Autoplay-on-scroll for feed videos (Reels/TikTok-style).** Video posts
in the "For You" feed now render as real inline `<video>` elements
(muted, no native controls) instead of a static thumbnail. A single
shared `IntersectionObserver` watches all of them: a video plays the
moment it's ~60% visible on screen and pauses the instant it scrolls
away — never more than one playing at a time, even if several are
technically in the DOM. Tapping a video toggles mute/unmute (a small
speaker icon shows the current state); a separate expand icon (⤢) opens
the full lightbox with sound, reactions, and comments. Switching tabs or
navigating away pauses everything automatically — CSS `display:none`
counts as "not intersecting," so there's no extra cleanup code needed,
and it can't leave a video quietly playing in the background.

This was deliberately built with scale in mind: `preload="metadata"`
(not `auto`) means scrolling past a video doesn't download the whole
file, only one video plays audio at a time, and R2's public URLs are
already served through Cloudflare's CDN so repeated plays of the same
clip don't hit your origin storage.

**Finished the Report/Block system**: comments in the lightbox now have
their own "Report" action (separate from reporting the whole post), and
a lightweight client-side cooldown (`throttled()`) now guards comment
(3s), testimonial (5s), and Community Value post (5s) submission —
stops the most common form of accidental spam (rapid double-tapping a
submit button on a slow connection) without needing a backend. Worth
being clear about the limit here: this is a **deterrent, not real rate
limiting** — a determined bad actor calling Firestore directly can still
bypass a client-side check. Actual enforcement needs either Cloud
Functions or routing writes through a backend (like `r2-media-api`
already does for uploads); worth revisiting if abuse patterns show up
that this doesn't stop.

## Phase 8 — Critical bugs (account loading, upload flicker) + anti-abuse foundation

**"Couldn't load your account" even on a fine connection — root cause
found and fixed.** `loadUserProfileWithRetry` only retried when a read
*threw* — but on a fresh registration, `onAuthStateChanged` fires and
tries to read the profile at almost the same instant the registration
handler is still writing that very same document. If the read wins that
race, `snap.exists()` is `false` — not an error — so the old code gave up
immediately, no matter how good the connection was. Fixed at two layers:
retries now cover "not found yet," not just thrown errors; and
`onAuthStateChanged` no longer overwrites a good `currentUserDoc` (set
directly by the registration handlers as a first line of defense) with a
stale, racy `null` for the same session.

**UI "twinkling" after uploading one file then immediately opening
another — root cause found and fixed.** If an upload is abandoned mid-
flight (e.g. the user hits back before it finishes) and a new one starts,
the first XHR keeps running in the background — SPAs don't cancel
in-flight requests just because the screen changed. Both uploads' progress
callbacks were fighting over the same shared progress-bar element, which
looked exactly like flickering until the abandoned one finally finished.
Fixed with a generation-counter guard on both Add Product and Archive
uploads: any DOM update from a superseded attempt is now silently ignored.

**Anti-abuse foundation, built for real (500k+ users) scale in mind:**
- **Firestore validation hardened**: product price/title, post captions,
  comments, and testimonials are now validated server-side (size limits,
  type checks, non-empty) — previously only enforced in the UI, which
  isn't real security since a client-side check can always be bypassed.
- **Reporting**: any post, comment, or user can be reported (reason +
  optional details) via a proper modal. Reports are admin-only readable —
  neither the reporter nor the reported party can see them. This is the
  mechanism every major platform actually leans on, since automated
  content moderation at this scale is a whole separate project on its own.
- **Blocking**: block/unblock any Builder from their Market page or
  directly from a feed card. Blocked users' content is filtered out of
  both Discover and the For You feed. This is a client-side content
  filter (hides what you see), not a full reciprocal access ban — a
  blocked user can still technically view/interact with your content
  unless stronger enforcement gets added later (would need extra `get()`
  checks on every read rule, which has a real cost at scale, so it's a
  deliberate v1 trade-off rather than an oversight).
- A "⋯" more-options menu now lives on the Builder Market header and
  every feed card for exactly this (Report / Block).
